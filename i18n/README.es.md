...
Campos del body para crear: `on_conflict` (`"ignore"` | `"update"`, omitido = error ante conflicto) y `conflict_columns` (array de nombres de columna, obligatorio para `"update"`) permiten inserts idempotentes contra una unique constraint — por ejemplo, reintentos de entrega de webhook. `"ignore"` no hace nada ante un conflicto (200 con la fila existente si se indicó `conflict_columns`, 204 en caso contrario); `"update"` hace upsert vía `ON CONFLICT DO UPDATE` (201 si realmente se insertó, 200 si se actualizó).
...
- **Tools expuestas:** 26 tools que cubren gestión de apps/tablas/columnas, claves foráneas, columnas enum, políticas de fila (plantillas y cláusulas avanzadas), modo RLS, webhooks, miembros, tokens e inspección de solo lectura — p. ej. `orbit_create_app`, `orbit_create_table`, `orbit_add_table_column`, `orbit_create_policy_from_template`, `orbit_set_table_rls_mode`, `orbit_create_webhook`. Mismo camino de validación, aprovisionamiento y auditoría que la REST API y el dashboard, sin atajos — lista completa y schemas vía descubrimiento de tools del MCP (`tools/list`).
...
`-p 1 -parallel 4` importa aquí: `internal/dashboard` e `internal/mcpserver` comparten la misma base de datos de prueba, y el paralelismo predeterminado de Go entre paquetes hace que uno interfiera con las filas del otro (deadlocks / violaciones de FK espurias, no bugs reales). Coincide con lo que ejecuta el CI — ver `CONTRIBUTING.md`.
...
Zeep Orbit usa un modelo de licencia dual. El núcleo (todo el repositorio,
excepto los directorios enterprise a continuación) es **MIT** — ver
[LICENSE](../LICENSE).

El código en `internal/enterprise/` y su espejo de frontend
`internal/dashboard/ui/src/enterprise/` es source-available bajo la
[Zeep Orbit Enterprise Source License](../internal/enterprise/LICENSE):
libre para leer, estudiar y modificar, pero el uso en producción requiere una
Clave de Licencia Enterprise activa. Ver [LICENSING.md](../LICENSING.md)
para el resumen del modelo, [docs/docs/enterprise-licensing.md](../docs/docs/enterprise-licensing.md)
para una explicación orientada al producto, y
[COMMERCIAL_TERMS.md](../COMMERCIAL_TERMS.md) para los términos de
suscripción.

Ninguna función enterprise existe todavía — esto solo establece el límite
de licencia antes del propio mecanismo de licenciamiento enterprise
