...
O Zeep Orbit vem com um servidor [Model Context Protocol](https://modelcontextprotocol.io) para que assistentes de código com IA criem e inspecionem apps, tabelas, políticas de linha, membros, tokens e webhooks diretamente — sem precisar clicar no dashboard.
...
- **Autenticação:** dois métodos, ambos resolvidos contra o mesmo store de Personal Access Token:
  ...
  - **OAuth 2.1 + PKCE** — registro dinâmico de client, fluxo authorization code, rotação de refresh token, discovery em `/.well-known/oauth-authorization-server`. É o que o Claude Desktop usa no fluxo interativo de conexão.
- **Tools expostas:** 26 tools cobrindo gerenciamento de apps/tabelas/colunas, chaves estrangeiras, colunas enum, políticas de linha (templates e cláusulas avançadas), modo RLS, webhooks, membros, tokens e inspeção somente-leitura — ex.: `orbit_create_app`, `orbit_create_table`, `orbit_add_table_column`, `orbit_create_policy_from_template`, `orbit_set_table_rls_mode`, `orbit_create_webhook`. Mesmo caminho de validação, provisionamento e auditoria da REST API e do dashboard, sem atalhos — lista completa e schemas via descoberta de tools do MCP (`tools/list`).

### Configuração do cliente

Gere um PAT primeiro (**Dashboard → MCP**), exporte como variável de ambiente e configure seu cliente:
...
> Trate o PAT como uma senha — nunca commite. Se seu cliente não suportar interpolação de variável `${VAR}` no arquivo de config, mantenha esse arquivo fora do controle de versão.
...
`--reuse-values` mantém os values já configurados no release (secrets, config) — não precisa
ter um `values.yaml` local à mão; use `--values values.yaml` no lugar se for mudar alguma config.
`-n <namespace>` precisa bater com o namespace onde o release foi instalado (confira com
`helm list -A`). A flag `--atomic` faz rollback automático se a atualização falhar.

O `image.tag` padrão é `latest`, então só o `helm upgrade` não recria os pods se a string da tag
não mudou — o Kubernetes só faz rollout quando o pod spec muda. Force o pull da nova imagem com:
...
Em produção, fixe `image.tag` ou `image.digest` no seu `values.yaml` em vez de depender do
...
`-p 1 -parallel 4` importa aqui: `internal/dashboard` e `internal/mcpserver` compartilham o mesmo banco de teste, e o paralelismo padrão do Go entre pacotes faz um atrapalhar as linhas do outro (deadlocks / violações de FK espúrias, não bugs reais). Bate com o que o CI roda — ver `CONTRIBUTING.md`.
...
exceto os diretórios enterprise abaixo) é **MIT** — veja [LICENSE](../LICENSE).

O código em `internal/enterprise/` e seu espelho de frontend
...
livre para ler, estudar e modificar, mas uso em produção exige uma Chave de
Licença Enterprise ativa. Veja [LICENSING.md](../LICENSING.md) para o
overview do modelo, [docs/docs/enterprise-licensing.md](../docs/docs/enterprise-licensing.md)
para uma explicação orientada a produto, e [COMMERCIAL_TERMS.md](../COMMERCIAL_TERMS.md)
para os termos de assinatura.

Nenhuma feature enterprise existe ainda — isso só estabelece o limite de
licença antes do próprio mecanismo de licenciamento enterprise
