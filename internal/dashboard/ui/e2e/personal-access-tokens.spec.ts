...
/**
 * Personal Access Tokens settings UI (.specs/features/mcp-server, T16).
 *
 ...
 *
 * Stages map to T16's Done-when: create a token (assert plaintext shown
 * once) -> reload (assert value hidden, entry still listed) -> confirm the
 ...
 *
 * The Personal Access Tokens dialog is opened via the sidebar footer's
 * "key" icon button (PersonalAccessTokens.tsx, wired in DashboardShell.tsx)
 * — there is no dedicated Settings page for this in the current UI, the
 * same modal pattern the existing change-password action uses.
 */
...
test('create, reload, authenticate, and revoke a personal access token', async ({ page }) => {
  ...
  // Stage 1: open the Personal Access Tokens dialog and create a token —
  // its plaintext must be shown exactly once (T16 Done-when).
  ...
  const revealedToken = await page.locator('code').innerText()
