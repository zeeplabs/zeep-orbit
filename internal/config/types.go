...
type ColumnConfig struct {
	...
	// DefaultIsExpression marks Default as a raw SQL expression (e.g.
	// "now()", "gen_random_uuid()") instead of a literal value. Only
	// expressions on the defaultExpressions allowlist (validate.go) are
	// accepted — this is embedded unquoted into DDL, so it is never
	// treated as arbitrary user SQL.
	DefaultIsExpression bool             `json:"default_is_expression,omitempty" yaml:"default_is_expression,omitempty"`
