...
| `CREATE INDEX` locking behavior | Out of scope to change in this spec — the new `orbit_add_table_index` tool's docstring/description SHALL warn the calling agent (and, by extension, its operator) that index creation is a blocking operation on the target table, matching `ensureIndexes`'s current non-concurrent behavior. Actually switching to `CREATE INDEX CONCURRENTLY` (which requires running outside a transaction and can't be part of the same `Apply` batch as other DDL) is a real backend change deferred to design.md as an explicit open question, not decided here | n — flagged for design.md, not resolved |
...
**Open questions:** none blocking — the `CREATE INDEX CONCURRENTLY` question above is logged as an assumption (interim: keep current blocking behavior, documented to the caller) and deferred to design.md, not left unresolved here.
