...
**Issues found**: none blocking. Two minor observations were raised (not spec deviations or surviving mutants) and have since been closed with dedicated tests, gate re-run clean:
1. ~~No dedicated test exercises the FK-cycle-detection path (`detectReferenceCycle`) through the new `AddTableColumnForUser` endpoint specifically~~ — closed by `TestAddTableColumnForUser_ReferenceCompletingCycleRejected` (`internal/dashboard/apps_table_column_foruser_test.go`): table `a` references `b`, then adding a column to `b` referencing `a` completes the cycle and is rejected with a `*ValidationError`, table `b` left untouched.
2. ~~No dedicated test for the superadmin/`CanReadAnyApp` edge case on the 4 new tools~~ — closed by 4 new tests, one per tool's `*ForUser` function: `TestAddTableColumnForUser_SuperadminBypassesMembership`, `TestAddTableIndexForUser_SuperadminBypassesMembership`, `TestCreateWebhookForUser_SuperadminBypassesMembership`, `TestSaveEventMappingForUser_SuperadminBypassesMembership` — each calls the function with a superadmin identity carrying no `app_members` row at all, confirming `ResolveAppRole`'s superadmin bypass (`AppRoleAdmin`) satisfies both `CanWrite()` and `CanManage()`.

**Next steps**: None. Both gaps closed, `go test ./internal/dashboard/... -race` re-run clean (5 new tests + full suite).
