...
| Enforcement por RLS em armazenamento de arquivo (`/{app}/files/*`, `internal/server/storage_handler.go`) | Arquivos são servidos via S3/MinIO, sem tabela Postgres própria endereçável pelo builder de policies (`_files` não é uma app-table em `app_tables`); não há RLS nativo pra aplicar. `storage_handler.go` continua rodando como role owner, fora do escopo do AC-04 da story "Bypass interno" — nenhum bug, apenas fora do que esta feature endereça. |
...
4. The system SHALL garantir que nenhum handler de tabela de app-schema no caminho `/{app}/...` (usuário final) — `HandleList`/`HandleGet`/`HandleGetByID`/`HandleInsert`/`HandleUpdate`/`HandleDelete` em `internal/server/handler.go` — executa código com o role owner — a troca de role para `<role_enduser>` é incondicional nesse subconjunto de handlers, não uma opção configurável por request. Escopo explicitamente restrito a tabelas de app-schema com RLS nativo (o único tipo de tabela endereçável pelo builder de policies desta feature); não se aplica a `internal/server/storage_handler.go` (`/{app}/files/*`), que serve arquivos via S3/MinIO e não tem, hoje, tabela Postgres própria com RLS — ver Out of Scope.
