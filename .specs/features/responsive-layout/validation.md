...
4. ~~Login rate limiting: running all 7 tests of this file in one `npx playwright test` process reliably trips the backend login limiter (each test logs in fresh)~~ — **Closed** (post-release, D-239 follow-up): added `e2e/global-setup.ts` + Playwright `storageState` so the suite logs in once and reuses the session; `login()` now only submits the form when a spec genuinely starts unauthenticated. Full-suite local re-run: 0 rate-limit failures (was the suggested fix here — implemented).
