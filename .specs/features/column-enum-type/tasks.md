...
**Status**: ✅ Complete (with two SPEC_DEVIATIONs, both noted inline as code comments in `TableCard.tsx`: (1) design.md's claim of an "existing FK edit-action UI pattern (FormDrawer/ConfirmDialog) already used for FK edit" in this file does not match the actual code — verified by reading `TableCard.tsx` in full: FK `references` changes go through the same generic save-all-columns form as every other column field, there is no dedicated FK edit action to mirror. Built the new "edit allowed values" action from `FormDrawer` directly (already imported/used elsewhere in this file for the relationships/index info panels) instead. (2) The task's "Where" didn't list `internal/dashboard/ui/src/lib/api.ts`, but `SchemaEditor` needed an `appId` prop threaded through from `TableCard` (already available there) to build the PATCH URL — a prop-plumbing change confined to this same component tree, not a new exported API surface; the actual PATCH call is a plain inline `fetch` in `TableCard.tsx` itself, not a new `lib/api.ts` hook, to keep the touched-file set as close to the task's listed scope as possible.)
