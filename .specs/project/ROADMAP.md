...
- [x] "Edit with AI" — expande o chat de IA pra editar app já existente (add tabela/coluna/índice/FK-em-coluna-nova, toggle RLS por tabela, toggle auth do app), reusando os handlers `*ForUser` já existentes; rename/drop coluna, drop índice e FK-em-coluna-existente ficam fora (sem handler seguro hoje) — spec: `.specs/features/ai-edit-chat/` (implementado e verificado, ver `validation.md`; ainda não liberado em release)
